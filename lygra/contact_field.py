...
from lygra.utils.vis_utils import make_arrow_lines, show_visualization
...
class ContactField:
    def __init__(self):
        ...
        self.all_specific_patch_keyvectors = []
        self.specific_patch_ids =[]
    ...
    def reduce_link_interaction(self, interaction_matrix):
        ...
        result = []
        func_links = []
        for i,idxes in enumerate(self.all_contact_link_contact_field_idxes):
            result.append(interaction_matrix[:, idxes, :].bool().any(dim=1).int())
            if set(idxes) & set(self.specific_patch_ids):
               func_links.append(i)
        return torch.stack(result, dim=1),torch.tensor(func_links).cuda().long()
    ...
    def register_keyvector_specific(self, link_name, keyvectors, centroid):
        '''
            Static keyvector is for the palm...
        ...
        '''
        # we assume this is already in the hand base frame.
        self.all_specific_patch_keyvectors.append(keyvectors)
        return
    ...
    def sample_spatial_specific_contact(self, batch_size):
        # Default fallback option
        return self.accel_structure.sample_spatial_specific_contact(batch_size)
    ...
    def generate_contact_field(self, tree, n_iter=80000, device='cuda'):
        ...
        if len(self.all_specific_patch_keyvectors) > 0:
            self.all_specific_patch_keyvectors = np.concatenate(self.all_specific_patch_keyvectors) # [n, 6]
    ...
    def generate_acceleration_structure(self, method='lbvhs2', **kwargs):
        ...
        if method == 'lbvhs2':
            structure = LBVHS2ContactFieldAccelStructure(orgs, dirs, self.field_vec_id, angle_range,self.specific_patch_ids, **kwargs)
...
def has_aligned(query, vecs, threshold=math.pi*0.49999):
    query_norm = query / np.linalg.norm(query)
    vecs_norm = vecs / np.linalg.norm(vecs, axis=-1, keepdims=True)
    
    dots = (query_norm * vecs_norm).sum(axis=-1)
    return np.any(dots > math.cos(threshold))
    ...
    for link_name, mesh_group in link_mesh_group.items():
        ...
        for i in range(n_group):
            ...
            if link_name in rule["static_link"]:
                ...
                # movable link.
                passed = True
                if not additional_filter_func(link_name, centroid):
                    passed = False
                ...
                if passed:
                    ...
                    if link_name in rule["specific_link"]:
                        contact_field.register_keyvector_specific(link_name, keyvector, centroid)
    contact_field.specific_patch_ids.extend([i for i, val in enumerate(contact_field.all_patch_parent_link_names) if val in rule["specific_link"]])
