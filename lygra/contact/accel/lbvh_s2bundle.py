...
class LBVHS2ContactFieldAccelStructure:
    def __init__(self, origin, direction, ids, angle_range, specific_patch_ids, box_w=0.01, max_leaf_size=256):
        self.contact_lbvhs = [] 
        self.all_boxes_min = []
        self.all_boxes_max = []
        self.specific_boxes_min = []
        self.specific_boxes_max = []
        ...
        for i in tqdm(range(len(origin)), desc="Building Contact Field BVH"):
            ...
            if i in specific_patch_ids:
                self.specific_boxes_min.append(boxes_min)
                self.specific_boxes_max.append(boxes_max)
        self.all_boxes_min = torch.cat(self.all_boxes_min)
        self.all_boxes_max = torch.cat(self.all_boxes_max)
        self.specific_boxes_min = torch.cat(self.specific_boxes_min)
        self.specific_boxes_max = torch.cat(self.specific_boxes_max)
    ...
    def sample_spatial_specific_contact(self, n):
        point = sample_points_from_box_union(self.specific_boxes_min, self.specific_boxes_max, n)
        normal = torch.randn(n, 3).to(self.device)
        normal = F.normalize(normal, dim=-1)
        return np.concatenate([point.detach().cpu().numpy(), normal.detach().cpu().numpy()], axis=-1)
