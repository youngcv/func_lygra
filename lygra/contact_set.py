...
def sample_independent_set(n, initial_mask, dependency_matrix,mandatory_indices=None):
    """
    ...
    mandatory_mask = torch.zeros_like(mask)
    mandatory_mask[:, mandatory_indices] = 1
    all_samples = []

    for i in range(n):
        if i == 0:
            current_sampling_mask = mask * mandatory_mask
        else:
            current_sampling_mask = mask
        sampled = sample_from_mask(current_sampling_mask)  # [B]
