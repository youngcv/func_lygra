...
class KinematicsTree:
    ...
    def build_from_urdf(urdf_path, active_joint_names=None):
        ...
        if active_joint_names is None:
            active_joints = [joint for joint in urdf.joints if joint.joint_type != "fixed"]
    ...
    def get_self_collision_check_link_pairs(
        self,
        link_body_id,
        whitelist_pairs=None,
        whitelist_link=None,
    ):
        whitelist_pairs = whitelist_pairs or []
        whitelist_link = whitelist_link or []
        n_link = self.n_link()
    ...
    def set_fixed_zero_joints(self, joint_names):
        if not joint_names:
            return

        missing = [name for name in joint_names if name not in self.active_joints]
        if missing:
            raise ValueError(
                f"fixed_zero_joints must be active joint names. Unknown/inactive joints: {missing}. "
                f"Available active joints: {self.active_joints}"
            )

        for name in joint_names:
            joint_id = self.joints.index(name)
            self.joint_limit_lowers[joint_id] = 0.0
            self.joint_limit_uppers[joint_id] = 0.0
...
def build_kinematics_tree(
    urdf_path='assets/hand/left_sharpa_ha4/left.urdf',
...
def batch_jacobian(
    ...
    for i, (joint_id, link_id) in enumerate(jacobian_pairs):
        ...
        if joint_type == 'revolute':
            ...
            out[active_joint_id, link_id * 6:link_id * 6 + 3, :] = joint_axis_pose_in_base.transpose(-1, -2)
...
def compute_dq_new_tall(J, dx, regularization):
