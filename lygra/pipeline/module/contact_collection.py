...
def sample_pose_and_contact_from_interaction(
    ...
    n_sample_repeat=4,
    min_contact_domain_size=1,
    accept_downsamle = False,
    thumb_indices=None
):
    ...
    success = False
    while n_contact >= 2:
        contact_ids = sample_independent_set(n_contact, initial_mask, dependency_matrix, thumb_indices)                # [n_batch_outer, n_contact]
        valid_batch_idx = torch.where((contact_ids >= 0).all(dim=-1))
        if len(valid_batch_idx[0]) > 0:
            break
        else:
            if not accept_downsamle:
                sys.exit(-1)
            # too hard, resample.
            print("Resampling")
            n_contact -= 1 
