...
def get_object_pose_sampling_args(strategy, mask , use_static_prob ,robot):
    args = {
        "strategy": strategy,
        "func_mask": mask,
...
def sample_object_pose_kernel(
    ...
    if sampling_args["strategy"] == 'specific' :
        func_mask = sampling_args["func_mask"]
        func_points = points[func_mask]
        func_normals = normals[func_mask]
        robot_contact = contact_field.sample_spatial_specific_contact(n)
        sample_idx = np.random.randint(0, len(func_points), n)
        sampled_object_contact_points = func_points[sample_idx]
        sampled_object_contact_normals = func_normals[sample_idx]
        ...
        func_points = func_points.unsqueeze(0).expand(result.shape[0], -1, -1)
        rotation = result[:, :3, :3]
        translation = result[:, :3, 3]  # [n, 3]

        point_transformed = torch.bmm(func_points, rotation.transpose(-1, -2)) + translation.unsqueeze(1)    # [n, n_point, 3]
...
def sample_object_pose(
    ...
    use_static_prob=sampling_args["use_static_prob"]
